    ...
    Credentials({
      ...
      async authorize  (credentials: any) :Promise<any>{
        ...
        if (!user[0]) {
          ...
          throw new AccountNotLinked("User not found. Please register.", new AccessDenied("User not found"));
        } else {
          // in this secttion we check user password
          const isPassCorrect = await isPasswordCorrect(userPass, user[0].password);
          ...
          if (isPassCorrect) {
            ...
            throw new AccessDenied("Password is incorrect", new AccessDenied("Password is incorrect"));
        }
        }
      },
    }),
